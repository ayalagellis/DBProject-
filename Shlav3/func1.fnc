...
BEGIN
    -- Find community with the most births in the last 3 years
    SELECT cm.community_id,
           c.community_type,
           COUNT(b.birth_id) AS num_of_births
    INTO v_community_id_births, v_community_type_births, v_num_of_births
    FROM birth b
    ...
    ORDER BY COUNT(b.birth_id) DESC FETCH FIRST 1 ROWS ONLY;
    ...
    -- Find community that spent the most amount on food
    SELECT c.community_id,
           c.community_type,
           SUM(s.amount) AS total_spending_on_food
    INTO v_community_id_food, v_community_type_food, v_amount_spent_on_food
    FROM spending s
    ...
    ORDER BY SUM(s.amount) DESC FETCH FIRST 1 ROWS ONLY;
